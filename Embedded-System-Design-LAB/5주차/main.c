...
//@TODO - 1 Set the clock, (//) 주석 표시를 없애고 틀린 값이 있다면 제대로 된 값으로 수정하시오 
    ...
    // HCLK를 나누지 않겠다.
    // PPRE2는 APB2
    RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE2_DIV2; //SYSCLK 32, PCLK2를 16MHZ으로 만들어줘야해서 2로 나눠줘야함.
    ...
    RCC->CFGR2 |= (uint32_t)(RCC_CFGR2_PREDIV2_DIV5 | RCC_CFGR2_PLL2MUL8 | RCC_CFGR2_PREDIV1_DIV10 | RCC_CFGR2_PREDIV1SRC_PLL2);
...
//@TODO - 4 GPIO Configuration
  ...
  GPIOA->CRH |= (uint32_t)(GPIO_CRH_CNF8_1 | GPIO_CRH_MODE8_0);
  /* USART Pin Configuration */
  GPIOA->CRH |= (uint32_t)(GPIO_CRH_CNF9_1 | GPIO_CRH_MODE9_0 | GPIO_CRH_CNF10_1);
...
int main() {
  ...
  while (1) {
    ...
    if((GPIOD->IDR & SELECT_NUMBER) != SELECT_NUMBER){
      delay();
      for(i = 0; i < 15; i++){
        SendData(msg[i]);
      }
